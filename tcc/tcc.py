...
def conectar_banco():
    try:
        ...
        return None
# Função para salvar o código de barras no banco de dados
def salvar_codigo_no_banco(filme, cadeira, codigo_barra, nome_cliente):
    try:
        conexao = conectar_banco()
        if conexao:
            cursor = conexao.cursor()

            # Inserir o código de barras no banco de dados
            cursor.execute("""
                INSERT INTO codigos_barra (filme, cadeira, codigo_barra, nome_cliente)
                VALUES (%s, %s, %s, %s)
            """, (filme, cadeira, codigo_barra, nome_cliente))

            conexao.commit()
            cursor.close()
            conexao.close()
    except mysql.connector.Error as err:
        messagebox.showerror("Erro", f"Erro ao salvar o código de barras no banco de dados: {err}")
...
def gerar_codigos_de_barras(nome, filme, cadeiras, quantidade):
    try:
        ...
        for cadeira in cadeiras:
            ...
            # Salvar o código de barras no banco de dados
            salvar_codigo_no_banco(filme, cadeira, codigo, nome)
